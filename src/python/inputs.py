...
for j in range(len(Comp1)):
    for lentght in range(len(Comp1)):
        Comp3.append("(" + str(Comp1[j]) + " * " + str(Comp1[lentght]) + ")")
        Comp3.append("(" + str(Comp1[j]) + " ^ " + str(Comp1[lentght]) + ")")
        Comp3.append("(" + str(Comp1[j]) + " v " + str(Comp1[lentght]) + ")")
...
for i in range(len(Possth2)):
    for j in range(len(Var)):
        if (
            "(0 * (" in Possth2[i]
            or "v (" + str(Var[j]) + " * " + str(Var[j]) + "))" in Possth2[i]
            or "((" + str(Var[j]) + " * " + str(Var[j]) + ") v" in Possth2[i]
            or "* (" + str(Var[j]) + " * " + str(Var[j]) + "))" in Possth2[i]
        ):
...
for i in range(len(Possth3)):
    for j in range(len(Var)):
        if (
            "(0 * ((" in Possth3[i]
            or ") v (" + str(Var[j]) + " * " + str(Var[j]) + "))" in Possth3[i]
            or "((" + str(Var[j]) + " * " + str(Var[j]) + ") v (" in Possth3[i]
            or ") * (" + str(Var[j]) + " * " + str(Var[j]) + "))" in Possth3[i]
        ):
